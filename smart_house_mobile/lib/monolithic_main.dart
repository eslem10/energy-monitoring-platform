...
String defaultApiUrl() => kIsWeb ? 'http://127.0.0.1:3001' : 'http://10.0.2.2:3001';
...
class _AuthPageState extends State<AuthPage> {
  ...
  Future<void> submit() async {
    ...
    try {
      final path = registerMode ? '/api/auth/register' : '/api/auth/login';
      final response = await http
          .post(
            Uri.parse('${widget.apiUrl}$path'),
      ...
      if (!mounted) return;
      setState(() {
        error = 'Backend auth unavailable: ${err.toString()}';
      });
    } finally {
      if (mounted) {
...
class _DashboardShellState extends State<DashboardShell> {
  ...
  Uri get chatbotEndpoint {
    final parsed = Uri.tryParse(apiUrl);
    if (parsed == null || parsed.host.isEmpty) {
      return Uri.parse('http://127.0.0.1:8000/chat');
    }

    return parsed.replace(port: 8000, path: '/chat', query: '');
  }

  void updateApiUrl(String value) {
    ...
    widget.onApiUrlChanged(normalized);
    refreshAll();
  }
  ...
  Future<void> refreshAll({bool silent = false}) async {
    try {
      final results = await Future.wait([
        ...
        getJson('/api/alerts/history?limit=100'),
      ...
      setState(() {
        ...
        energy = (results[2] as List)
            ...
            .toList();
        alerts = (results[4] as List)
            .map((item) => AppAlert.fromJson(item))
            .toList();
        dailyStats = DailyStats.fromJson(results[5]);
        diagnostics = Map<String, dynamic>.from(results[6] as Map);
  ...
  Future<void> markAlertRead(String id) async {
    ...
    try {
      final response = await http.post(endpoint('/api/alerts/$id/read'));
      if (response.statusCode < 200 || response.statusCode >= 300) {
        throw Exception(response.body);
      }
  ...
  Future<void> toggleFavorite(String deviceName) async {
    setState(() {
    ...
    try {
      await http.post(
        ...
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'favorites': favoriteDevices.toList()}),
      );
    } catch (_) {}
  }
  ...
  Future<void> _loadCustomScenes() async {
    try {
      final state = await getJson('/api/app-state');
      if (!mounted || state is! Map) return;

      final favorites = state['favorites'];
      final scenes = state['scenes'];
      setState(() {
        if (favorites is List) {
          favoriteDevices = favorites.map((item) => '$item').toSet();
        }
        if (scenes is List) {
          customScenes = scenes
              .whereType<Map>()
  ...
  Future<void> deleteCustomScene(String name) async {
    ...
    try {
      await http.delete(endpoint('/api/scenes/${Uri.encodeComponent(name)}'));
      await _persistCustomScenes();
    } catch (_) {}
  }
  ...
  Widget build(BuildContext context) {
    final pages = [
      ...
      SettingsPage(
        ...
        apiUrl: apiUrl,
        onApiUrlChanged: updateApiUrl,
        onAddDevice: addDevice,
        onDeleteDevice: deleteDevice,
        onRefresh: refreshAll,
        onLogout: widget.onLogout,
      ),
      DiagnosticsPage(diagnostics: diagnostics, error: error, onRefresh: refreshAll),
      NotificationsPage(alerts: alerts, onRefresh: refreshAll, onMarkRead: markAlertRead),
      ChatbotPage(
        summary: summary,
        history: history,
        energy: energy,
        alerts: alerts,
        chatbotUrl: chatbotEndpoint,
      ),
    ];
...
class DashboardPage extends StatelessWidget {
  ...
  final Future<void> Function(String deviceName) onToggleFavorite;
...
class HistoryPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        const SectionHeader(title: 'Daily stats'),
        const SizedBox(height: 8),
        Row(
          children: [
            Expanded(
              child: MetricCard(
                title: 'Energy 24h',
                value: '${(dailyStats?.todayWh ?? 0).toStringAsFixed(2)} Wh',
            ...
            Expanded(
              child: MetricCard(
                title: 'Vs yesterday',
                value: dailyStats?.changePercent == null
                    ? '--'
                    : '${dailyStats!.changePercent! >= 0 ? '+' : ''}${dailyStats!.changePercent!.toStringAsFixed(1)}%',
        ...
        Row(
          children: [
            Expanded(
              child: MetricCard(
                title: 'Average 24h',
                value: formatPower(dailyStats?.averagePower ?? 0),
                icon: Icons.speed_rounded,
              ),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: MetricCard(
                title: 'Peak 24h',
                value: formatPower(dailyStats?.peakPower ?? 0),
                icon: Icons.trending_up_rounded,
              ),
            ),
          ],
        ...
        const SizedBox(height: 16),
        const SectionHeader(title: 'Top devices today'),
        const SizedBox(height: 8),
        if ((dailyStats?.byDevice ?? const <DeviceEnergy>[]).isEmpty)
          const EmptyCard(text: 'No daily stats yet')
        else
          ...(dailyStats?.byDevice ?? const <DeviceEnergy>[]).take(5).map(
                (item) => ListTileCard(
...
class _ChatbotPageState extends State<ChatbotPage> {
  ...
  Future<void> ask(String question) async {
    ...
    if (text.isEmpty || isWaitingForReply) return;

    setState(() {
      isWaitingForReply = true;
      messages.add(ChatMessage(text: text, fromUser: true, time: DateTime.now()));
      messages.add(ChatMessage(text: 'Thinking...', fromUser: false, time: DateTime.now()));
    });
    ...
    try {
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        throw Exception('Chat backend returned ${response.statusCode}');
      }

      final data = jsonDecode(response.body) as Map<String, dynamic>;
      final reply = '${data['response'] ?? ''}'.trim();

      if (!mounted) return;
      setState(() {
        messages[messages.length - 1] = ChatMessage(
          text: reply.isEmpty ? 'The assistant returned an empty response.' : reply,
          fromUser: false,
          time: DateTime.now(),
        );
      ...
      setState(() {
        messages[messages.length - 1] = ChatMessage(
          text: 'I cannot reach the chatbot backend right now. Make sure FastAPI is running on ${widget.chatbotUrl}.',
          fromUser: false,
          time: DateTime.now(),
        );
        isWaitingForReply = false;
                ...
                children: quickQuestions
                    .map(
                      (item) => ActionChip(
                        ...
                        onPressed: isWaitingForReply ? null : () => ask(item),
                      ),
                    )
                    .toList(),
              ),
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    onSubmitted: isWaitingForReply ? null : ask,
                  ),
                ),
                const SizedBox(width: 10),
                IconButton.filled(
                  onPressed: isWaitingForReply ? null : () => ask(inputController.text),
...
class DiagnosticsPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final memory = diagnostics?['memory'];
    final rss = memory is Map ? numberOrNull(memory['rss']) ?? 0 : 0;
    final memoryMb = rss / 1024 / 1024;
    final mqttConnected = diagnostics?['mqttConnected'] == true;
    final uptime = numberOrNull(diagnostics?['uptime']) ?? 0;
          ...
          Row(
            children: [
              Expanded(
                child: MetricCard(
                  title: 'Node API',
                  value: error == null ? 'OK' : 'DOWN',
                  icon: Icons.api_rounded,
                  color: error == null ? const Color(0xff0a7d55) : Colors.red,
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: MetricCard(
                  title: 'MQTT',
                  value: mqttConnected ? 'Connected' : 'Check',
                  icon: Icons.hub_rounded,
                  color: mqttConnected ? const Color(0xff0a7d55) : Colors.orange,
                ),
              ),
            ],
          ),
          ...
          Row(
            children: [
              ...
              Expanded(
                child: MetricCard(
                  title: 'Uptime',
                  value: '${uptime.toStringAsFixed(0)} s',
    ...
    final items = [
      ...
      _MoreItem(
        ...
        page: SettingsPage(
          devices: adminDevices,
          apiUrl: apiUrl,
          onApiUrlChanged: onApiUrlChanged,
...
class _SettingsPageState extends State<SettingsPage> {
  ...
  void initState() {
    super.initState();
    apiUrlController = TextEditingController(text: widget.apiUrl);
  }
        ...
        CardBox(
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Use your PC IP for a real phone, example: http://192.168.31.82:3001.',
              ),
              const SizedBox(height: 12),
              TextField(
                ...
                decoration: const InputDecoration(
                  labelText: 'Node API URL',
                  prefixIcon: Icon(Icons.link_rounded),
                ),
              ),
              const SizedBox(height: 12),
              FilledButton.icon(
                onPressed: () => widget.onApiUrlChanged(apiUrlController.text),
...
class AlertTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTileCard(
      ...
      title: alert.title,
      subtitle:
          '${prettyDevice(alert.device)} - ${alert.reason}${alert.resolved ? ' - Resolved' : ''}',
      trailing: Column(
        mainAxisSize: MainAxisSize.min,
        ...
        children: [
          ...
          Text(
            timeOnly(alert.lastSeenAt ?? alert.time),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('First seen: ${alert.firstSeenAt ?? alert.time}'),
          Text('Last seen: ${alert.lastSeenAt ?? alert.time}'),
          Text('Occurrences: ${alert.count}'),
          Text('State: ${alert.resolved ? 'Resolved' : 'Active'}'),
          Text('Read: ${alert.acknowledged ? 'Yes' : 'No'}'),
        ],
      ),
    );
  }
...
class DailyStats {
  ...
  final String? peakDevice;
  final DateTime? peakTime;
  final List<DeviceEnergy> byDevice;

  factory DailyStats.fromJson(dynamic json) {
    final byDevice = json is Map ? json['byDevice'] : null;

    return DailyStats(
      todayWh: json is Map ? numberOrNull(json['todayWh']) ?? 0 : 0,
      yesterdayWh: json is Map ? numberOrNull(json['yesterdayWh']) ?? 0 : 0,
      averagePower: json is Map ? numberOrNull(json['averagePower']) ?? 0 : 0,
      peakPower: json is Map ? numberOrNull(json['peakPower']) ?? 0 : 0,
      changePercent: json is Map ? numberOrNull(json['changePercent']) : null,
      peakDevice: json is Map && json['peakDevice'] != null
          ? '${json['peakDevice']}'
          : null,
      peakTime: json is Map && json['peakTime'] != null
          ? parseTime(json['peakTime'])
          : null,
      byDevice: byDevice is List
          ? byDevice.map((item) => DeviceEnergy.fromJson(item)).toList()
          : const <DeviceEnergy>[],
    );
  }
}
...
class AppAlert {
  ...
  factory AppAlert.fromJson(dynamic json) {
    return AppAlert(
      ...
      count: (numberOrNull(json['count']) ?? 1).round(),
      firstSeenAt: json['firstSeenAt'] == null ? null : parseTime(json['firstSeenAt']),
      lastSeenAt: json['lastSeenAt'] == null ? null : parseTime(json['lastSeenAt']),
    );
  }
}
